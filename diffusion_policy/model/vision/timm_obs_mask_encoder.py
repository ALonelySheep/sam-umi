...
class TimmObsMaskEncoder(ModuleAttrMixin):
    def __init__(self,
            shape_meta: dict,
            model_name: str,
            pretrained: bool,
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == 'rgb':
                ...
                # SAM mask
                if attr.get('mask'):
                    mask_in_shape = np.prod(shape)
                    mask_out_shape = 768
                    print(f"{key}: Input shape of mask: {mask_in_shape}, output shape of mask {mask_out_shape}")
                    this_model = torch.nn.Sequential(
                        nn.Flatten(start_dim=1),
                        nn.Linear(in_features=mask_in_shape, out_features=mask_out_shape),
                    )
                    key_model_map[key] = this_model

                # camera rgb
                else:
                    print(f"Using pretrained model for {key}")
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            ...
            torch.set_rng_state(state) # ensure the same transform for concurrent input (rgb and mask)
...
if __name__=='__main__':
    timm_obs_encoder = TimmObsMaskEncoder(
        shape_meta=None,
